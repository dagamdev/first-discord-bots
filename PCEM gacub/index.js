...
const servidorID = "773249398431809586", creadorID = "717420870267830382", colorErr = "#ff0000", emojiError = "<a:negativo:856967325505159169>", rolesPrincipales = ["823372926707171358","887450254826418236","852684847901704192","830260569012699146","840704377962758204","887443737804931122","885301022677942272","887443742469029961"]
...
client.on("ready", async () => {
    ...
    .setTitle("<a:afirmativo:856966728806432778> Encendido de nuevo.")
    .setColor("#00ff00")
    .setFooter(client.user.username,client.user.displayAvatarURL())
    .setTimestamp()
    client.channels.cache.get("964696468181094410").send({embeds: [embEncendido]})
    ...
    function presencias () {
        ...
        let tiempo = new Date()
        if(tiempo.getHours()-5 > 22 || tiempo.getHours()-5 < 8){
            ...
            for(let d=0; d<dataCrc.prisioneros.length; d++){
                ...
                if(!miembro){
                    ...
                    if((dataCrc.prisioneros[d].tiempo + msTime) - tiempoActual <= 0){
                        ...
                        .setAuthor({name: miembro.user.tag, iconURL: miembro.displayAvatarURL({dynamic: true})})
                        ...
                        .setColor("#00ff00")
                        .setFooter({text: servidor.name, iconURL: servidor.iconURL({dynamic: true})})
                        .setTimestamp()
    ...
    async function colaboradores(){
        ...
        arrayCo.filter(f=>f.colaborador).forEach(async (col, ps) => {
            ...
            .setColor(servidor.members.cache.get(col.id).displayHexColor)
    ...
    function mensajesTemporales(){
        ...
        canales.map(m=> servidor.channels.cache.get(m).send("***<a:gatito:909919946140684288> ¡Hola!***").then(tms=> setTimeout(()=> {tms.delete()}, 2000)))
    }
    // mensajesTemporales()
    ...
    async function slashCommands(){
        ...
        let comandos = [ayudaCmd, reglasCmd, pingCmd, informacionCmd.toJSON(), clasificacionesCmd.toJSON(), crearCmd.toJSON(), historialCmd.toJSON(), finalizarCmd.toJSON(), nuevoCmd.toJSON(), rerollCmd, ascenderCmd, degradarCmd, avatarCmd, examenCmd, estadisticasCmd, sugerirCmd, marcarCmd, plantillaCmd, limpiarCmd, encarcelarCmd, expulsarCmd, banearCmd, desbanearCmd, websCmd,  usuarioCmd]
        comandos.forEach(async (comando, posicion) => {
            ...
            // await servidor.commands.delete("968205925867470950").then(dc=> console.log(`Comando ${dc.name} eliminado`))
    }
    slashCommands()
})
...
client.on("interactionCreate", async int => {
    ...
    if(int.isCommand()){
        if(int.commandName == "ayuda"){
            ...
            .setAuthor({name: `Hola ${int.user.username}`, iconURL: int.user.displayAvatarURL({dynamic: true})})
        ...
        if(int.commandName == "clasificaciones"){
            ...
            if(int.options.getSubcommand("invitaciones") == "invitaciones"){
                int.deferReply()
                estadisticas.comandos++
                let dataIn = await invitacionesDB.findById(servidorID), ordenado = dataIn.miembros.filter(f=> f.totales>0).sort((a,b)=> b.totales - a.totales), topIn = [], segPage = null
                for(o in ordenado){
        ...
        if(int.commandName == "información"){
            ...
            if(int.options.getSubcommand("miembro") == "miembro"){
                int.deferReply()
                estadisticas.comandos++
                let dataAli = await alianzasDB.findById(servidorID), dataSug = await systemSug.findById(servidorID), dataTs = await ticketsDB.findById(servidorID), dataInv = await invitacionesDB.findById(servidorID), miembro = false
                ...
                if(miembro){
                    ...
                    if(miembro.id == int.user.id){
                        if(miembro.permissions.has("ADMINISTRATOR") || miembro.roles.cache.has("887444598715219999")){
                            ...
                            setTimeout(async ()=>{
                                await int.editReply({embeds: [embInfoMiembro]})
                            }, 400)
                    ...
                    if(int.member.permissions.has("ADMINISTRATOR")){
                        ...
                        setTimeout(async ()=>{
                            await int.editReply({ephemeral: true, embeds: [embInfoMiembro]})
                        }, 400)
                    }
                }
            }
        ...
        if(int.commandName == "crear"){
            ...
            if(int.options.getSubcommand("colaborador") == "colaborador"){
                estadisticas.comandos++
                let dataCol = await colaboradoresDB.findById(servidorID), arrayCo = dataCol.colaboradores, miembro = int.guild.members.cache.get(int.options.getUser("colaborador").id), nombre  = int.options.getString("nombre")
        ...
        if(int.commandName == "nuevo"){
            if(int.options.getSubcommand("cazador-alianzas") == "cazador-alianzas"){
                ...
                const embRegistro = new Discord.MessageEmbed()
                .setAuthor({name: `Ejecutado por ${int.user.tag}`, iconURL: int.user.displayAvatarURL({dynamic: true})})
                ...
                .setColor("#00F0FF")
                .setFooter({text: miembro.user.username, iconURL: miembro.displayAvatarURL({dynamic: true})})
                .setTimestamp()
        ...
        if(int.commandName == "historial"){
            if(int.options.getSubcommand("colaboradores") == "colaboradores"){
                ...
                if(tabla.length > 10){
                    ...
                    setTimeout(async ()=>{
                        await int.editReply({embeds: [embColaboradores], components: [botones1]}).then(async msg =>{
                            await int.channel.messages.fetch(msg.id, {force: true}).then(mensaje => {
                                const colector = mensaje.createMessageComponentCollector({filter: u=> u.user.id == int.user.id, time: segPage*60000})
            ...
            if(int.options.getSubcommand("personal") == "personal"){
                ...
                if(tabla.length > 10){
                    ...
                    .setDescription(descripcion+tabla.slice(ttp1,ttp2).join("\n\n"))
                    .setColor(int.guild.me.displayHexColor)
                    .setFooter({text: `Pagina ${pagina}/${segPage}`, iconURL: footerURL})
                    .setTimestamp()
                    ...
                    setTimeout(async ()=>{
                        await int.editReply({embeds: [embHitorial], components: [botones1]}).then(async msg =>{
                            await int.channel.messages.fetch(msg.id, {force: true}).then(mensaje => {
                                const colector = mensaje.createMessageComponentCollector({filter: u=> u.user.id == int.user.id, time: segPage*60000})
        ...
        if(int.commandName == "avatar"){
            ...
            if(miembro){
                ...
                if(miembro.id == int.user.id){
                    ...
                    .setImage(miembro.displayAvatarURL({dynamic: true, size: 4096}))
                    .setColor(int.guild.me.displayHexColor)
                    .setFooter({text: miembro.tag, iconURL: miembro.displayAvatarURL({dynamic: true})})
                    .setTimestamp()
                    setTimeout(()=>{
                        int.editReply({allowedMentions: {repliedUser: false}, embeds: [embAvaMe]})
                    }, 400)
                ...
                if(usID){
                    await client.users.fetch(usID, {force: true}).then(usuario=> {
                        ...
                        .setColor(int.guild.me.displayHexColor)
                        .setFooter({text: usuario.tag, iconURL: usuario.displayAvatarURL({dynamic: true})})
                        .setTimestamp()
                        setTimeout(()=>{
                            int.editReply({allowedMentions: {repliedUser: false}, embeds: [embAvaMe]})
                        }, 400)
                    ...
                    const embAvaMe = new Discord.MessageEmbed()
                    .setAuthor({neme: int.user.tag, iconURL: int.user.displayAvatarURL({dynamic: true})})
        ...
        if(int.commandName == "sugerir"){
            ...
            .setAuthor({name: `${emojiError} Error`})
        ...
        if(int.commandName == "ascender"){
            ...
            if(!miembro.roles.cache.has(dataPer.datos.rolID)){
                miembro.roles.add(dataPer.datos.rolID)
            }
            persona.rango++
            ...
            .setColor("#00C624")
            .setFooter({name: miembro.user.username, iconURL: miembro.displayAvatarURL({dynamic: true})})
            .setTimestamp()
            int.guild.channels.cache.get(dataPer.datos.canalRegistro).send({embeds: [embRegistro]})
            await personalDB.findByIdAndUpdate(servidorID, {personal: arrayPr})
                    ...
                    async function clearMessages(){
                        ...
                        if(bueltas == 1 && filtro.length==0){
                            ...
                            if(mensajes == cantidad || (bueltas > 1 && filtro.length==0)){
                                ...
                                const embRegistro = new Discord.MessageEmbed()
                                .setAuthor({name: `Comando ejecutado por ${int.user.tag}`, iconURL: int.user.displayAvatarURL({dynamic: true})})
        ...
        if(int.commandName == "encarcelar"){
            ...
            .setColor("#ECDE03")
            .setFooter({text: `Incumpliste alguna regla de ${int.guild.name}`, iconURL: int.guild.iconURL({dynamic: true})})
            .setTimestamp()
        ...
        if(int.commandName == "expulsar"){
            ...
            .setAuthor({name: int.member.nickname ? int.member.nickname: int.user.username, iconURL: int.user.avatarURL({dynamic: true})})
            .setThumbnail(miembro.displayAvatarURL({dynamic: true, format: "png"||"gif", size: 1024}))
            .setColor("#ff8001")
            .setTimestamp()
            ...
            .setFooter({text: `Por el moderador: ${int.user.tag}`, iconURL: int.user.displayAvatarURL({dynamic: true})})
            .setColor("#ff8001")
            .setTimestamp()
            ...
            if(int.user.id == int.guild.ownerId){
                ...
                if(miembro.bot){
                    ...
                    miembro.kick(`Moderador ID: ${int.user.id} | Miembro expulsado: ${miembro.tag}, ID: ${miembro.id} | Razón: ${razon}`).then(k=>{
                        miembro.send({embeds: [embExpulsarDM]}).then(tdm=>{
                            ...
                            embExpulsar
                            .setFooter({text: `No he podido enviar el mensaje al exmiembro ${miembro.user.tag}`, iconURL: miembro.displayAvatarURL({dynamic: true})})
                            setTimeout(()=>{
                                int.editReply({embeds: [embExpulsar]})
                            }, 400)
                        })
                ...
                if(miembro.bot){
                    ...
                    int.guild.members.cache.get(miembro.id).kick(`Moderador ID: ${int.user.id} | Miembro expulsado: ${miembro.tag}, ID: ${miembro.id} | Razón: ${razon}`).then(k=>{
                        ...
                        miembro.send({embeds: [embExpulsarDM]}).catch(c=> console.log(c))
                    })
                    miembro.kick(`Moderador ID: ${int.user.id} | Miembro expulsado: ${miembro.tag}, ID: ${miembro.id} | Razón: ${razon}`).then(k=>{
                        miembro.send({embeds: [embExpulsarDM]}).then(tdm=>{
                            embExpulsar
        ...
        if(int.commandName == "banear"){
            ...
            await client.users.fetch(miembro ? miembro.id: id, {force: true}).then(async usuario=> {
                ...
                .setAuthor({name: usuario.tag, iconURL: usuario.displayAvatarURL({dynamic: true})})
                .setThumbnail(int.guild.iconURL({dynamic: true, format: "png"||"gif", size: 1024}))
    ...
    if(int.isButton()){
        ...
        if(int.customId == "aprobar"){
            ...
            if(miembro){
                ...
                .setColor("#00ff00")
                .setFooter({text: `Sugerencia aceptada por ${int.user.tag}`, iconURL: int.user.  displayAvatarURL({dynamic: true})})
                .setTimestamp()
                ...
                const embSugerencia = new Discord.MessageEmbed()
                .setAuthor({name: `Nueva sugerencia de ${miembro.user.tag}`, iconURL: miembro.displayAvatarURL({dynamic: true})})
                ...
                const embSugerencia = new Discord.MessageEmbed()
                .setAuthor({naem: `Nueva sugerencia de ${usuario.tag}`, iconURL: usuario.displayAvatarURL({dynamic: true})})
                .setTitle("✉️ Sugerencia:")
                .setDescription(`${dataSug.mensajes[posicion].sugerencia}`)
                .setColor(rolSugerencia.hexColor)
                .setTimestamp()
        ...
        if(int.customId === "denegar"){
            ...
            if(miembro){
                ...
                .addField(`✉️ **Sugerencia:**`, `${dataSug.mensajes[posicion].sugerencia}`)
                .setColor(colorErr)
                .setFooter({text: `Sugerencia denegada por ${int.user.tag}`, iconURL:int.user.displayAvatarURL({dynamic: true})})
                .setTimestamp()
        ...
        if(int.customId == "implementada"){
            ...
            if(miembro){
                ...
                .addField("✉️ **Sugerencia:**", `${dataSug.mensajes[posicion].sugerencia}`)
                .setColor("#00ff00")
                .setFooter({text: `Actualizado por ${int.user.tag}`, iconURL: int.user.displayAvatarURL({dynamic: true})})
        ...
        if(int.customId == "crearTicket"){
            ...
            if(dataTs.tickets.some(s=> s.miembroID == int.user.id) && dataTs.tickets.find(f=> f.miembroID == int.user.id).cerrado == false){
                ...
                int.guild.channels.create(`『🎫』ticket ${dataTs.datos.cantidad}`, {parent: "833120722695487518", permissionOverwrites: [{id: int.guildId, deny: "VIEW_CHANNEL"}, {id: int.user.id, allow: ["VIEW_CHANNEL", "SEND_MESSAGES"]}, {id: "831669132607881236", allow: ["VIEW_CHANNEL", "SEND_MESSAGES"]}, {id: "773271945894035486", allow: ["VIEW_CHANNEL", "SEND_MESSAGES"]}], topic: `<:candadoAbierto:962574461054373929> Ticket número **${dataTs.datos.cantidad}** de <@${int.user.id}> que se unió al servidor <t:${Math.floor(int.member.joinedAt / 1000)}:R> creado el <t:${Math.floor(Date.now() / 1000)}:F> *(<t:${Math.floor(Date.now() / 1000)}:R>)* esta *abierto*.`, reason: `Nuevo ticket número ${dataTs.datos.cantidad} creado por ${int.user.tag}.`}).then(async ch=>{
        ...
        if(int.customId == "finalizar"){
            let dataTs = await ticketsDB.findOne({_id: servidorID}), arrayTs = dataTs.tickets, arrayMs = dataTs.miembros, estrella = "<:estrella1:963478022369980517>", estrellaVacia = "<:estrellavacia:963486547179634729>", descripcion = int.channel.topic.split(" ")
            ...
            arrayTs.forEach(async (objeto) => {
                if(objeto.id == int.channelId){
                    ...
                    if(objeto.valoracion == false){
                        ...
                        .setColor(int.guild.me.displayHexColor)
                        .setFooter({text: personal.nickname ? personal.nickname: personal.user.username, iconURL: personal.displayAvatarURL({dynamic: true})})
                        .setTimestamp()
...
client.on("guildMemberAdd",async gmd => {
    if(gmd.guild.id != servidorID) return;
    ...
    if(gmd.user.bot){
        if(!gmd.user.flags.has("VERIFIED_BOT")){
            ...
            .setAuthor({name: gmd.user.tag, iconURL: gmd.user.displayAvatarURL({dynamic: true})})
            ...
            .setFooter({text: gmd.guild.name, iconURL: gmd.guild.iconURL({dynamic: true})})
            .setTimestamp()
        ...
        .setAuthor({name: gmd.guild.name, iconURL: gmd.guild.iconURL({dynamic: true})})
        .setThumbnail(gmd.displayAvatarURL())
        .setTitle("🤖 Se unio un Bot")
        ...
        .setColor(`${gmd.guild.me.displayHexColor}`)
        .setFooter({text: `Bienvenido/a a ${gmd.guild.name}`, iconURL: gmd.guild.iconURL({dynamic: true})})
        .setTimestamp()

        let usBanner = await client.users.fetch(gmd.id, {force: true})
        let embBien = new Discord.MessageEmbed()
        .setAuthor({name: gmd.user.tag, iconURL: gmd.user.displayAvatarURL({dynamic: true})})
        ...
        await gmd.guild.invites.fetch().then(async invites=> {
            let invi = invites.find(f=> arrayMi.find(fm=> fm.id==f.inviterId) ? arrayMi.find(fm=> fm.id==f.inviterId).codes.find(fc=> fc.code==f.code) ? arrayMi.find(fm=> fm.id==f.inviterId).codes.find(fc=> fc.code==f.code).usos<f.uses : false : false)
            
            if(invi){
                let miembro = arrayMi.find(f=> f.id==invi.inviterId)
                if(miembro){
                    if(miembro.codes.some(s=> s.code==invi.code)){
                        let invite = miembro.codes.find(f=> f.code==invi.code)
                        if(invi.uses > invite.usos){
                            if(miembro.id == gmd.user.id){
                                miembro.falsas++
                                embBienvenida.description = `*Has sido invitado/a por ti mismo con una invitación creada por ti.*\n\n💈 Pásate por el canal <#823639152922460170> en el podrás obtener roles que cambiarán el color de tu nombre dentro del servidor, y muchos otros roles.\n\n📢 Promociona todo tipo de contenido en el canal **<#836315643070251008>**.\n\n📜 También pásate por el canal <#823343749039259648> el canal de reglas, léelas para evitar sanciones.`
                                ...
                                let miembroSV = gmd.guild.members.cache.get(invi.inviterId)
                                ...
                                embBienvenida.description = `*Has sido invitado/a por <@${miembro.id}> quien ahora tiene **${miembro.verdaderas.toLocaleString()}** ${miembro.verdaderas==1 ? "invitación": "invitaciones"}.*\n\n💈 Pásate por el canal <#823639152922460170> en el podrás obtener roles que cambiarán el color de tu nombre dentro del servidor, y muchos otros roles.\n\n📢 Promociona todo tipo de contenido en el canal **<#836315643070251008>**.\n\n📜 También pásate por el canal <#823343749039259648> el canal de reglas, léelas para evitar sanciones.`
...
client.on("guildMemberRemove",async gmr => {
    if(gmr.guild.id != servidorID) return;
    ...
    if(gmr.user.bot){
        ...
        let embDes = new Discord.MessageEmbed()
        .setAuthor({name: gmr.user.username, iconURL: gmr.user.displayAvatarURL({dynamic: true, format: "png"||"gif", size: 4096})})
        ...
        .setColor("#ff0000")
        .setFooter({text: gmr.guild.name, iconURL: gmr.guild.iconURL({dynamic: true})})
        .setTimestamp()
...
client.on("inviteCreate", async invC => {
    if(invC.guild.id != servidorID) return;
    ...
    if(miembro){
        ...
        arrayMi.push({id: invC.inviterId, tag: client.users.cache.get(invC.inviterId).tag, verdaderas: 0, totales: 0, restantes: 0, falsas: 0, tiempo: undefined, codes: [{code: invC.code, usos: 0}], invitados: []})
        await invitacionesDB.findByIdAndUpdate(servidorID, {miembros: arrayMi})
    }
})

client.on("inviteDelete", async invD => {
    if(invD.guild.id != servidorID) return;
...
client.on("messageCreate",async msg => {
    ...
    // Sistema VIP
    if(msg.channelId == "826193847943037018" && msg.mentions.everyone && msg.member.roles.cache.has("826197551904325712") && !msg.member.permissions.has("ADMINISTRATOR")){
        msg.channel.permissionOverwrites.edit(msg.author.id, {"MENTION_EVERYONE": false,})
    }
...
client.on("messageCreate", async msg => {
    ...
    if(comando == "expulsarpersonal" && msg.author.id == creadorID){
        ...
        if(miembro){
            ...
            const embRegistro = new Discord.MessageEmbed()
            .setAuthor({name: `Ejecutado por el dueño`, iconURL: int.user.displayAvatarURL({dynamic: true})})
...
client.on("guildBanAdd",async gba => {
    if(gba.guild.id != servidorID) return;
...
client.on("guildBanRemove", async gbr => {
    if(gbr.guild.id != servidorID) return;
...
client.on("channelDelete", async cd => {
    ...
    let dataTs = await ticketsDB.findOne({_id: servidorID}), arrayTs = dataTs.tickets, objetoDs = dataTs.datos, servidor2 = client.guilds.cache.get("949860813915705354"),  descripcion = cd.topic.split(" ")
...
process.on("unhandledRejection", async err => {
    ...
    .setColor("ff0000")
    .setTimestamp()
    client.channels.cache.get(dataBot.datos.registros.errores).send({embeds: [embErr]})
})
